...
# Shadowsocks 默认端口
https_proxy=http://127.0.0.1:1080 http_proxy=http://127.0.0.1:1080 gemini [命令]
```
...
> **注意**：全局代理会影响所有需要网络的命令，包括不需要代理的国内服务。推荐使用单命令代理方式。
...
```bash
# 首选 Pro 模型
https_proxy=http://127.0.0.1:7890 gemini -m gemini-3.1-pro-preview --skip-trust -p "复杂分析" -o text

# 快速模型
https_proxy=http://127.0.0.1:7890 gemini -m gemini-3-flash-preview --skip-trust -p "简单任务" -o text

# Pro 失败自动切换 Flash（推荐）
cat file.js | https_proxy=http://127.0.0.1:7890 \
gemini -m gemini-3.1-pro-preview --skip-trust -p "审查" || \
gemini -m gemini-3-flash-preview --skip-trust -p "审查"
```
