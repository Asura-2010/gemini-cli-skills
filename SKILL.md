...
**重要**：使用单行方式设置代理，只对当前 gemini 命令生效，不影响其他不需要代理的命令：

```bash
# 推荐：代理只对这条 gemini 命令生效
...
```bash
# 国内环境带代理
https_proxy=http://127.0.0.1:7890 gemini "[prompt]" --yolo -o text 2>&1
```
...
```bash
https_proxy=http://127.0.0.1:7890 gemini "创建 [描述] 包含 [功能]。输出完整文件内容。" --yolo -o text
```

### 代码审查（管道方式推荐）

```bash
cat 文件路径 | https_proxy=http://127.0.0.1:7890 gemini -m gemini-3.1-pro-preview --skip-trust -p "审查提示词" || \
https_proxy=http://127.0.0.1:7890 gemini -m gemini-3-flash-preview --skip-trust -p "审查提示词"
```

### Bug修复

```bash
https_proxy=http://127.0.0.1:7890 gemini "修复 [文件] 中的这些bug：[列表]。立即应用修复。" --yolo -o text
```

### 测试生成

```bash
https_proxy=http://127.0.0.1:7890 gemini "为 [文件] 生成 [Jest/pytest] 测试。重点关注 [区域]。" --yolo -o text
```

### 文档生成

```bash
https_proxy=http://127.0.0.1:7890 gemini "为 [文件] 所有函数生成 JSDoc。输出 markdown 格式。" --yolo -o text
```

### 架构分析

```bash
https_proxy=http://127.0.0.1:7890 gemini "使用 codebase_investigator 分析此项目" -o text
```

### 网络研究

```bash
https_proxy=http://127.0.0.1:7890 gemini "[主题] 最新情况是什么？使用 Google Search。" -o text
```
...
gemini -m gemini-3.1-pro-preview --skip-trust -p "审查提示词" || \
gemini -m gemini-3-flash-preview --skip-trust -p "审查提示词"
```
...
# 2. 审查（Gemini 审查自己的工作）
https_proxy=http://127.0.0.1:7890 gemini "审查 [文件] 的 bug 和安全问题" -o text

# 3. 修复发现的问题
https_proxy=http://127.0.0.1:7890 gemini "修复 [文件] 中的 [问题]。立即应用。" --yolo -o text
```
...
在项目根目录创建 `.gemini/GEMINI.md` 作为持久上下文，Gemini 会自动读取。
